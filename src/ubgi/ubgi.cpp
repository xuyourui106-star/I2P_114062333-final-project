...
//     auto alive = [&](){
//         if(my_gen != g_search_gen.load()){
//             std::cerr << "[DBG] alive()=false: gen mismatch (my_gen=" << my_gen
//                       << " current_gen=" << g_search_gen.load() << ")\n";
//             return false;
//         }
//         if(g_ctx.stop){
//             ctx.stop = true;
//         }
//         if(ctx.stop){
//             std::cerr << "[DBG] alive()=false: ctx.stop is true\n";
//         }
//         return !ctx.stop;
//     };

//     std::cerr << "[DBG] do_search start: my_gen=" << my_gen
//               << " max_depth=" << max_depth
//               << " movetime_ms=" << movetime_ms << "\n";

//     if(state.legal_actions.empty()){
//         std::cerr << "[DBG] no legal actions at all, sending bestmove 0000\n";
//         if(alive()){
//             send("bestmove 0000");
//             g_bestmove_sent = true;
//         }
...
//     }
//     if(state.game_state == WIN){
//         std::cerr << "[DBG] game_state == WIN at root, sending first legal move\n";
//         if(alive()){
//             send("bestmove " + move_to_str(state.legal_actions[0]));
//             g_bestmove_sent = true;
//         }
...
//     for(int depth = 1; depth <= depth_limit; depth++){
//         std::cerr << "[DBG] loop top: depth=" << depth << " alive=" << alive() << "\n";
//         if(!alive()){
//             std::cerr << "[DBG] break A: not alive before search() call, depth=" << depth << "\n";
//             break;
//         }

//         auto depth_start = std::chrono::high_resolution_clock::now();
//         SearchResult result = g_algo->search(&state, depth, history, ctx);
//         std::cerr << "[DBG] search() returned: depth=" << depth
...
//         if(!alive() && depth > 1){
//             std::cerr << "[DBG] break B: not alive after search(), depth=" << depth << "\n";
//             break;
//         }
...
//         if(!alive()){
//             std::cerr << "[DBG] break C: not alive after info send, depth=" << depth << "\n";
//             break;
//         }
//         if(movetime_ms > 0 && total_ms * 2 >= movetime_ms){
//             std::cerr << "[DBG] break D: movetime budget exceeded, total_ms=" << total_ms
//                       << " movetime_ms=" << movetime_ms << "\n";
//             break;
//         }
//         if(result.score >= P_MAX - 100 || result.score <= M_MAX + 100){
//             std::cerr << "[DBG] break E: score near mate bound, score=" << result.score
//                       << " P_MAX=" << P_MAX << " M_MAX=" << M_MAX << "\n";
//             break;
//         }
//     }

//     std::cerr << "[DBG] loop exited. final alive()=" << alive()
//               << " ctx.stop=" << ctx.stop
//               << " best_move=" << move_to_str(best_move) << "\n";

//     if(alive()){
//         std::cerr << "[DBG] sending bestmove: " << move_to_str(best_move) << "\n";
//         send("bestmove " + move_to_str(best_move));
//         g_bestmove_sent = true;
//     }else{
//         std::cerr << "[DBG] NOT sending bestmove because alive()==false at the end!\n";
//     }
//     g_searching = false;
// }
...
//     std::cerr << "[DBG] cmd_go: max_depth=" << max_depth
//               << " movetime_ms=" << movetime_ms
//               << " infinite=" << infinite << "\n";
